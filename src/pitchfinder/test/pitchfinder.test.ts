...
const readFileAsync = (...pathParts: string[]): Promise<Buffer> => {
  const filePath = path.resolve(__dirname, ...pathParts);
...
const decode = async (buffer: Buffer): Promise<Float32Array> => {
  const decoded: WavDecoderData = await WavDecoder.decode(buffer);
...
xdescribe('Pitchfinder', () => {
  ...
  const pitchSamples: string[] = fs.readdirSync(`${__dirname}/pitches`);
  ...
  describe('AMDF minimum/maximum frequency parameters', () => {
    ...
    pitchSamples.forEach((fileName) => {
      ...
      params.forEach((freqs) => {
        ...
        it(`Detects ${type} wave at ${hz} hz with minimumFrequency ${minFreq} hz and maximumFrequency ${maxFreq} hz`, async () => {
          const buffer: Buffer = await readFileAsync('pitches', fileName);
          const audio: Float32Array = await decode(buffer);
          const pitch: number | null = detector(minFreq, maxFreq)(audio);
          if (pitch == null) {
            throw new Error('No frequency detected');
          }
      ...
      describe('quarter-note quantization', () => {
        ...
        it('Dynamic Wavelet misses the low frequencies and has accuracy issues', () => {
          return readFileAsync('melodies', sample)
            .then(decode)
            .then((data) =>
              Pitchfinder.frequencies(detectors.DynamicWavelet, data, {quantization: 1})
            )
            .then((frequencies) => {
              expect(frequencies.map(round)).toEqual([
                null,
...
//             .then(decode)
//             .then((data) =>
//               Pitchfinder.notes([detectors.YIN, detectors.AMDF, detectors.DynamicWavelet], data)
//             )
//             .then((notes) =>
//               expect(notes.map(getMIDI)).toEqual([
//                 60,
...
//             .then((notes) =>
//               expect(notes.map(getMIDI)).toEqual([null, null, 63, 65, 67, 69, 71, 72])
//             );
//         });
